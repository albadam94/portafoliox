import React from 'react'

const Habilidades = () => {
  return (
    <div>

        <h1 className='mt-28 text-5xl font-semibold font-inter leading-[56px] md:mx-12 lg:mx-20 xl:mx-80 text-custom-teal'>Habilidades</h1>

        <div className="h-56 grid grid-cols-3 gap-4 content-evenly font-medium text-white md:mx-12 lg:mx-20 xl:mx-80">
                <div>Diseño UX/UI</div>
                <div>Wireframing</div>
                <div>Adobe Creative Cloud</div>
                <div>Investigación de usuarios</div>
                <div>Design Thinking</div>
                <div>Diseño de espacios</div>
                <div>Prototipado</div>
                <div>Diseño de producto</div>
                <div>Desarrollo Web</div>
</div>

    </div>
  )
}

export default Habilidades